    ...
    _onAddRecord: function (ev) {
        ...
        if (this.editable || data.forceEditable) {
            if (!this.activeActions.create) {
                ...
                if (self.name === "room_lines" && self.value.data.length === 1) {
                $('.o_field_x2many_list_row_add').first().hide();}
                else {
                ...
                if (self.name === "room_lines") {self.creatingRecord = true;
                $('.o_field_x2many_list_row_add').first().hide()} else {
