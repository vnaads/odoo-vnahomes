...
class HotelRoomType(models.Model):
    ...
    child_ids = fields.One2many('hotel.room.type', 'categ_id',
                                'Child Categories')
    room_ids = fields.One2many('hotel.room', 'categ_id', string=_("Room"))
    room_count = fields.Integer(string=_('Room'), compute='compute_room', store=True)
    ...
    def action_view_room(self):
        view = self.env.ref('hotel.view_hotel_room_form')

        action = self.env.ref('hotel.action_hotel_room_form').read()[0]
        # po_ids = [] #
        # for r in self.sudo().purchase_order_ids:
        # po_ids.append(r.id)
        if len(self.sudo().room_ids) == 1:
            action['views'] = [(view.id, 'form')]
            action['view_id'] = view.id
            action['res_id'] = self.sudo().room_ids[0].id
        else:
            action['domain'] = [('id', 'in', self.sudo().room_ids.ids)]
        return action
...
class FolioRoomLine(models.Model):
    ...
    partner_id = fields.Many2one(string='Partner', related='folio_id.partner_id')
...
class HotelRoom(models.Model):
    ...
    status = fields.Selection([('available', 'Available'), ('holding', 'Holding'),
                               ('occupied', 'Occupied')],
    ...
    def write(self, vals):
        ...
        if 'isroom' in vals and vals['isroom'] is False:
            vals.update({'color': 2, 'status': 'holding'})
...
class HotelFolio(models.Model):
        ...
        if not 'service_lines' and 'folio_id' in vals:
            ...
            try:
                ...
                for rec in folio_id:
                    for room_rec in rec.room_lines:
                        prod = room_rec.product_id.name
                        room_obj = h_room_obj.search([('name', '=', prod)], limit=1)
                        room_obj.write({'isroom': False})
                        vals = {'room_id': room_obj.id,
                                'check_in': rec.checkin_date,
                                'check_out': rec.checkout_date,
    ...
    def write(self, vals):
        ...
        for rec in self:
            ...
            if len(list(new_rooms)) != 0:
                ...
                for rm in room_list:
                    room_obj = h_room_obj.search([('name', '=', rm.name)], limit=1)
                    room_obj.write({'isroom': False})
                    vals = {
                        'room_id': room_obj.id,
                        'check_in': rec.checkin_date,
            ...
            if len(list(new_rooms)) == 0:
                ...
                for rom in room_list_obj:
                    room_obj = h_room_obj.search([('name', '=', rom.name)], limit=1)
                    room_obj.write({'isroom': False})
    ...
    def action_confirm(self):
        for r in self:
            r.room_lines[0].room_id.status = 'occupied'
...
class HotelServices(models.Model):
    ...
    @api.multi
    @api.onchange('uom_id')
    def onchane_uom_id(self):
        for r in self:
            if r.uom_id:
                r.uom_po_id = r.uom_id.id
